...
void loop()
{
    #ifdef GIMBAL_CALIBRATION
        if(calStatus ){
            calibrationRun(AUX1_Arm, AUX3_value);
        }
    #endif
        ...
        Serial.println();
            ...
            if (loopCount > 500 && loopCount <= 505) { // repeat 5 packets to avoid bad packet, change rate setting or command
                ...
                if (currentSetting == 1 || currentSetting == 2) {
                    ...
                    crsfClass.crsfPrepareCmdPacket(crsfCmdPacket, ELRS_TLM_RATIO_COMMAND, 2);  // 0xFE for wifi Mode
                    crsfClass.CrsfWritePacket(crsfCmdPacket, CRSF_CMD_PACKET_SIZE);
                } else if (currentSetting == 4) {
                    crsfClass.crsfPrepareCmdPacket(crsfCmdPacket, ELRS_TLM_RATIO_COMMAND, 8);
                    crsfClass.CrsfWritePacket(crsfCmdPacket, CRSF_CMD_PACKET_SIZE);
                    //delay(100000);
                }
                loopCount++;
